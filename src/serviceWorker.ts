...
interface WorkboxPlugin {
  requestWillStart?: ({ request }: { request: Request }) => Promise<Response>;
  requestWillFail?: ({ request }: { request: Request }) => Promise<null>;
}
...
const bgSyncPlugin: WorkboxPlugin = {
  ...
  async requestWillFail({ request }: { request: Request }) {
    // Use type assertion to avoid property does not exist error
    (self.registration as any).sync.register('sync-forms');
    return null;
  },
};
