      ...
      if (code) {
        ...
        try {
          ...
          if (token) {
            ...
            throw new Error('No token available for demo. Please use the manual token entry.');
          }
        } catch (err) {
          console.error('OAuth error:', err);
          setError('Authentication failed. Please try again or use a personal access token.');
        } finally {
          setIsLoading(false);
        }
      }
  ...
  return (
    <div className="github-oauth">
      ...
      <div className="oauth-note">
        <p>
          <strong>Note:</strong> This is a simplified demo of GitHub OAuth. In a production application, 
          you would need a serverless function to securely exchange the authorization code for a token.
        </p>
      </div>
    </div>
  );
